...
serviceManagementCltr.vehicleReceived = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({
        message: "Vehicle received info saved successfully",
        data: newEntry,
      });
  } catch (error) {
    console.error(error);
...
serviceManagementCltr.vehicleReceivedDetails = async (req, res) => {
  ...
  try {
    const vehicleReceivedDetails = await VehicleReceived.find({
      partnerId: partnerId,
    });
    if (vehicleReceivedDetails.length === 0) {
      return res
        .status(404)
        .json({ error: "No vehicles received for your workshop." });
    }
    ...
    res
      ...
      .json({
        error:
          "Something went wrong while fetching the vehicle received details. Please try again later.",
      });
  }
};
...
serviceManagementCltr.vehicleServiceEstimation = async (req, res) => {
  ...
  try {
    ...
    const vehicleServiceEstimationImagesResult = await uploadFileToCloudinary(
      files.vehicleServiceEstimationImages[0],
      "vehicle-service-estimation-images"
    );
    const capturedImageResult = await uploadFileToCloudinary(
      files.capturedImage[0],
      "captured-image"
    );
  ...
  }
  {
    ...
    res
      .status(500)
      .json({ error: "Something went wrong while registering new partner" });
  }
};

//priliminary estimation approval display for deal display
serviceManagementCltr.priliminaryEstimationApprovalReject = async (
  req,
  res
) => {
...
serviceManagementCltr.customerVehicleReceivedDetails = async (req, res) => {
  ...
  try {
    const vehicleReceiving = await VehicleReceived.find({
    ...
    res
      .status(500)
      .json({
        error: "Something went wrong while fetching the vehicleReceiving",
      });
  }
};
